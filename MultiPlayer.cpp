	...
	while (myWindow.isOpen())
	{
		...
		if (!liveArrow.isDead) {
			...
			if (kolejnosc==0 && liveArrow.isInterecting(gracz1)) {
				gracz1.playerHP--;
				if (gracz1.playerHP < 10) {//wywolanie konca gry po 1 hicie bo nie bede tyle czekal :P tutaj bool oznacza czy wygral gracz pierwszy 
...
/*void MultiPlayer::gameOver(sf::RenderWindow & myWindow, Background myBackground, bool playerOneWins)
{
	sf::Font myFont;
	...
	endMessage.setPosition(sf::Vector2f(myWindow.getSize().x*0.2, myWindow.getSize().y*0.1));
